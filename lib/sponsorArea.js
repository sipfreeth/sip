...
export async function updateBookingApproval(bookingId, decision, reviewerLabel) {
  await supabase
    .from('slot_bookings')
    .update({ approval_status: decision, reviewed_by: reviewerLabel, reviewed_at: new Date().toISOString() })
    .eq('id', bookingId);
}

export { MAX_FILES_PER_SPONSOR, MAX_FILE_MB, WEEKS_TO_SHOW };
