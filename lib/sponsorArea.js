...
// เช็คว่า Office นี้ "แนะนำ" สำหรับธุรกิจประเภทนี้ไหม — ต้องมีข้อมูล Demographic ของ Office ก่อน ไม่งั้นเช็คไม่ได้ (ถือว่าไม่แนะนำ)
// เกณฑ์เพศ: ต้องมีสัดส่วนเพศที่ต้องการ >= 55% (ถือว่า "เอียงไปทางนั้นชัดเจน" ไม่ใช่แค่ก้ำกึ่ง) — 'any' ผ่านเสมอ
// เกณฑ์อายุ: ช่วงอายุของ Office ต้องมีส่วนที่ทับซ้อนกับช่วงอายุที่ธุรกิจนี้ต้องการ
export function isOfficeRecommendedFor(office, businessType, rulesMap) {
  const rule = rulesMap[businessType];
  if (!rule || !office.demographics_completed_at) return false;

  const total = (office.demo_male_count || 0) + (office.demo_female_count || 0) + (office.demo_other_count || 0);
  if (total === 0) return false;

  let genderMatch = true;
  if (rule.preferred_gender === 'female') genderMatch = (office.demo_female_count || 0) / total >= 0.55;
  else if (rule.preferred_gender === 'male') genderMatch = (office.demo_male_count || 0) / total >= 0.55;

  const ageMatch = office.demo_age_min != null && office.demo_age_max != null && office.demo_age_min <= rule.preferred_age_max && office.demo_age_max >= rule.preferred_age_min;
