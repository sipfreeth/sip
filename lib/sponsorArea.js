...
export async function saveSponsorContent({ sponsorId, fileName, filePath, fileType, creativeId }) {
  ...
  // ตรวจสอบอัตราส่วนภาพ 16:9 (ตามสเปคจอจริงที่ติดตั้ง — แนวนอน 16:9) ก่อนบันทึกเข้าคลัง
  // ถ้าไม่ตรง ลบไฟล์ที่เพิ่งอัปโหลดทิ้งทันที กันมีไฟล์ค้างอยู่ใน Storage โดยไม่มีใครใช้
  ...
  if (fileBlob) {
    ...
    // ถ้าหาขนาดไม่ได้ (dims เป็น null — ไฟล์รูปแบบพิเศษที่ Parser อ่านไม่ได้) จะปล่อยผ่านไป ไม่บล็อกโดยไม่จำเป็น
  }
...
export async function getAvailability(officeAccountId) {
  ...
  const { data: bookings } = await supabase
    .from('slot_bookings')
    .select('slot_number, week_start, payment_status, reserved_until, sponsor_id, sponsors(business_type)')
  ...
  for (const b of bookings || []) {
    ...
    bookedMap[`${b.slot_number}_${b.week_start}`] = { ...b, businessType: b.sponsors?.business_type || null };
  }
...
export async function updateBookingContent(bookingId, sponsorId, sponsorContentId) {
  // ล็อกเสมอตั้งแต่ยืนยันการจองแล้ว ไม่ว่าจะอยู่สถานะไหนก็ตาม (รอตรวจสอบ/อนุมัติแล้ว/ไม่ผ่าน)
  // Sponsor แก้เองไม่ได้อีกต่อไป ต้องแจ้งทีมงานผ่านแชท และการเปลี่ยนแปลงขึ้นอยู่กับดุลยพินิจของทีมงาน
  throw new Error('ไม่สามารถเปลี่ยนเนื้อหาเองได้หลังยืนยันการจองแล้ว กรุณาแจ้งทีมงานผ่านแชทเพื่อขอเปลี่ยนแทน (การเปลี่ยนแปลงขึ้นอยู่กับความเหมาะสมและดุลยพินิจของทีมงาน)');
}

// เก็บไว้เผื่อใช้ในอนาคต (ไม่ได้เรียกใช้แล้วตอนนี้ เพราะ Sponsor แก้เองไม่ได้อีกแล้ว)
async function _legacyUpdateBookingContent(bookingId, sponsorId, sponsorContentId) {
...
export async function searchSponsors(keyword) {
  const { data } = await supabase
    .from('sponsors')
    .select('id, sponsor_code, company_name, email, email_verified_at')
...
export async function getPendingBookings() {
  const { data } = await supabase
    .from('slot_bookings')
    .select('*, sponsors(company_name, sponsor_code, business_type), office_accounts(office_name), sponsor_content(file_name, file_path, file_type)')
    .eq('approval_status', 'pending')
    .eq('payment_status', 'paid') // แสดงเฉพาะรายการที่ชำระเงินแล้วเท่านั้น กันสับสนกับรายการที่ยังไม่จ่าย/หมดเวลา
...
// ---------- สถานะการแสดงผลบนจอจริง (เทียบกับ Feed ที่ CMS ดึงไปที่ /api/playlist.js — ใช้สูตรคำนวณสัปดาห์เดียวกันเป๊ะๆ) ----------
function getCurrentWeekStartForStatus() {
  const now = new Date();
...
// คืนสถานะการแสดงผลของ Booking หนึ่งรายการ — null ถ้ายังไม่ถึงเงื่อนไขขึ้นจอ (ยังไม่จ่ายเงิน/ยังไม่อนุมัติ)
export function getAiringStatus(booking) {
  if (booking.payment_status !== 'paid' || booking.approval_status !== 'approved') return null;

  const currentWeekStart = getCurrentWeekStartForStatus();
  const bookingWeek = new Date(booking.week_start).toISOString().slice(0, 10);

  if (bookingWeek === currentWeekStart) return 'now_playing';
  if (bookingWeek < currentWeekStart) return 'ended';
  return 'upcoming';
}

export const AIRING_STATUS_LABEL = {
  now_playing: { text: '🟢 กำลังเล่นอยู่', color: '#06c755' },
  ended: { text: '⚪ สิ้นสุดแล้ว', color: '#9ca3af' },
  upcoming: { text: '🔵 รอถึงคิว', color: '#2a78d6' },
};

// ---------- ประเภทธุรกิจ Sponsor (รายการคงที่) ----------
export const BUSINESS_TYPE_LABEL = {
  restaurant: '🍽️ ร้านอาหาร',
  beverage: '🥤 เครื่องดื่ม',
  fashion: '👗 แฟชั่น/เสื้อผ้า',
  beauty_health: '💄 ความงาม/สุขภาพ',
  finance: '💰 การเงิน/ประกัน',
  real_estate: '🏢 อสังหาริมทรัพย์',
  technology: '💻 เทคโนโลยี/บริการ',
  education: '🎓 การศึกษา',
  travel_hotel: '✈️ ท่องเที่ยว/โรงแรม',
  automotive: '🚗 ยานยนต์',
  other: '📦 อื่นๆ',
};

// ---------- ดู Slot อื่นในออฟฟิศเดียวกัน สัปดาห์เดียวกัน ว่าเป็นธุรกิจประเภทไหนบ้าง (ใช้ตอน Admin ตรวจ Content) ----------
// ไม่ส่งชื่อบริษัท/ข้อมูลอื่นกลับไปเลย ส่งแค่ slot_number + business_type เท่านั้น
export async function getOfficeSlotCategories(officeAccountId, weekStart, excludeBookingId) {
  let query = supabase
    .from('slot_bookings')
    .select('id, slot_number, sponsors(business_type, company_name)')
    .eq('office_account_id', officeAccountId)
    .eq('week_start', weekStart)
    .eq('payment_status', 'paid')
    .neq('approval_status', 'rejected'); // เห็นทั้งที่อนุมัติแล้วและกำลังรอตรวจ (จ่ายเงินแล้วทั้งคู่) ยกเว้นที่ถูกปฏิเสธไปแล้ว
  ...
  return (data || []).map((row) => ({
    ...
    businessType: row.sponsors?.business_type || null,
    companyName: row.sponsors?.company_name || null,
  }));
}

// ---------- กฎจับคู่ประเภทธุรกิจ Sponsor กับ Demographic ที่เหมาะสม ----------
export async function getDemographicRules() {
  const { data } = await supabase.from('business_type_demographic_rules').select('*');
  const map = {};
  for (const row of data || []) map[row.business_type] = row;
  return map;
}

export async function saveDemographicRule(businessType, { preferredGender, ageMin, ageMax }) {
  const { error } = await supabase
    .from('business_type_demographic_rules')
    .upsert({ business_type: businessType, preferred_gender: preferredGender, preferred_age_min: ageMin, preferred_age_max: ageMax }, { onConflict: 'business_type' });
  if (error) throw error;
}

// เช็คว่า Office นี้ "แนะนำ" สำหรับ Sponsor รายนี้ไหม — ต้องมีข้อมูล Demographic ของ Office ก่อน ไม่งั้นเช็คไม่ได้ (ถือว่าไม่แนะนำ)
// ใช้ Target ที่ Sponsor กรอกเองเป็นหลัก (แม่นยำกว่า) ถ้ายังไม่ได้กรอกค่อย Fallback ไปใช้กฎมาตรฐานตามประเภทธุรกิจ (rulesMap)
// เกณฑ์เพศ: ต้องมีสัดส่วนเพศที่ต้องการ >= 55% (ถือว่า "เอียงไปทางนั้นชัดเจน" ไม่ใช่แค่ก้ำกึ่ง) — 'any' ผ่านเสมอ
// เกณฑ์อายุ: ช่วงอายุของ Office ต้องมีส่วนที่ทับซ้อนกับช่วงอายุเป้าหมาย
export function isOfficeRecommendedFor(office, sponsor, rulesMap) {
  if (!office.demographics_completed_at) return false;

  const total = (office.demo_male_count || 0) + (office.demo_female_count || 0) + (office.demo_other_count || 0);
  if (total === 0) return false;

  const rule = rulesMap[sponsor.business_type];
  const targetGender = sponsor.target_gender || rule?.preferred_gender || 'any';
  const targetAgeMin = sponsor.target_age_min ?? rule?.preferred_age_min ?? 18;
  const targetAgeMax = sponsor.target_age_max ?? rule?.preferred_age_max ?? 65;

  let genderMatch = true;
  if (targetGender === 'female') genderMatch = (office.demo_female_count || 0) / total >= 0.55;
  else if (targetGender === 'male') genderMatch = (office.demo_male_count || 0) / total >= 0.55;

  const ageMatch = office.demo_age_min != null && office.demo_age_max != null && office.demo_age_min <= targetAgeMax && office.demo_age_max >= targetAgeMin;
