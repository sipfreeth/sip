...
// คืน array 3 ช่อง (slot 1,2,3) เสมอ ไม่ว่าจะมีข้อมูลจริงกี่อันก็ตาม (ช่องว่างคือ null)
...
export async function getSlots(officeAccountId) {
  ...
  const slots = [1, 2, 3].map((n) => bySlot[n] || { slot_number: n });
...
export function renderOfficeAreaContent({ officeAccount, slots, canEdit, uploadUrlAction, saveAction, supabaseUrl, supabaseAnonKey }) {
  ...
  return `
    <div class="section">
      ...
      <p class="hint">Office Area — จัดการ Content 3 Slot</p>
    </div>
    ${slotCards}
    ${script}`;
}
