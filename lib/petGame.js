...
export async function giveTreat(memberId, shopItemId) {
  ...
  // เช็ค Point คงเหลือจาก points_ledger เดิม (ก้อนเดียวกับที่ใช้แลกของรางวัลจริง)
  const { data: ledger } = await supabase.from('points_ledger').select('reward_points').eq('member_id', memberId);
  const pointBalance = (ledger || []).reduce((sum, r) => sum + (r.reward_points || 0), 0);
