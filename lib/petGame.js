...
export async function useInventoryItem(memberId, inventoryId) {
  ...
  const config = await getConfig();
  const expBase = item.item_type === 'treat' ? config.exp_treat : config.exp_feed;
  // ป่วยอยู่ = ให้อาหารได้ปกติ (เพิ่มความอิ่ม/ความสุขได้) แต่ไม่ได้ EXP จนกว่าจะรักษาหาย
  const expGained = pet.isSick ? 0 : applyHappinessBonus(expBase, pet.happiness, config);
  const newHunger = Math.min(100, pet.hunger + item.hunger_boost);
  const newHappiness = Math.min(100, pet.happiness + item.happiness_boost);
  const newExp = pet.exp + expGained;
  ...
  await checkAndAwardBadges(pet.id, config, newExp);
  return { expGained, newHunger, newHappiness, newExp, itemName: item.name };
}
